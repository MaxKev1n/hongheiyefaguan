...
void mainform::changehead(int a)
{
    ...
    if(result == QMessageBox::Yes){
        n = headnumber == 1 ? tempnumber : headnumber - 1;
        rol = false;
    }
    else{
        n = headnumber == tempnumber ? 1 : headnumber + 1;
        rol = true;
    }
}
...
void mainform::send()
{
    ...
    QString tn = QString("%1号玩家").arg(p[n - 1].number);
    input->clear();
...
void mainform::toupiao(QListWidget *listWidget, QVector<player> p)
{
    ...
    QString s = QString("%1号玩家").arg(temp[0].number);
...
void mainform::change()
{
    ...
    playername->setText(s);
    if(!p[n-1].life){
        QString temp = QString("系统：%1号玩家已死亡").arg(n);
